                      ...
                      int line, struct progressbar *pb)
{
    ...
    if (pb->type == SKIN_TOKEN_VOLUMEBAR)
    {
        ...
        /* The number and the bar have to agree, and on this target the
         * number is a plain percentage of the -70..-10 dB scale (see
         * format_sound_value_ex()). to_normalized_volume() applies a
         * loudness curve on top of that, so 70% on the settings screen drew
         * a bar at about half - two different answers to the same question.
         * The scale here was already cut to the part of the range worth
         * having, which is the job the curve would otherwise be doing. */
        length = maxvol - minvol;
        end = global_status.volume - minvol;
#elif defined(HAVE_PERCEPTUAL_VOLUME) || defined(HAVE_TOUCHSCREEN)
        length = 1000;
